...
export async function synchroniserWidgetAjoutRapide(
  ...
  try {
    ...
    // de debug pour comparer avec ce que le natif reçoit réellement en cas
    // de crash à ce palier.
    // RÈGLE À NE JAMAIS CASSER — CORRECTIF DU 2026-09-14 (revue sécurité) :
    // gardé derrière __DEV__, jamais inconditionnel — ce palier est celui
    // livré en production (ETAPE_DEBUG_AJOUT_RAPIDE=4, cf. RÈGLE plus haut),
    // contrairement aux paliers 1-3 (debug pur, jamais livrés tels quels),
    // et loggait en clair des données financières réelles (montant/nom de
    // la dernière dépense) dans les logs device, y compris en TestFlight/
    // production — récupérables via Console.app/sysdiagnose.
